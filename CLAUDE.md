...
**Orbit / held poses** — `sneak` and `fly` are held poses; `orbit` spins `playerWrapper` independently. Held pose + no orbit → single-frame GIF. `?gifSize=256&gifFrames=8` URL knob renders tiny GIFs for the smoke test; production defaults (`DEFAULT_GIF_SIZE=512`, `DEFAULT_FRAMES=30`) apply when absent.
...
**Settings** — persisted to `localStorage` via `src/lib/settings.ts`. `panoramaSource` is the only key so far.
